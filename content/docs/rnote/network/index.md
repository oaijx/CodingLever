---
title: 网络协议实战：从分层模型到具体协议解析
description: 深入浅出解析网络协议的核心原理，涵盖分层模型、IP、TCP/UDP、DHCP、PXE 及 SDN 等实战内容。
keywords: 网络协议, TCP/IP 分层, DHCP, PXE, SDN, 技术成长
---
...
- ip 地址
    + a. 大小
		+ ipv4 : 4bytes
		+ ipv6 : 16bytes
	+ b. [地址类型](/docs/rnote/network/graph/ip地址分类.jpg)
...
- PXE (Pre-boot Execution Environment, 预启动执行环境)
    ...
    ![工作流](/docs/rnote/network/graph/PXE流程.jpg)
...
- ICMP 报文格式
    ![ICMP 报文格式](/docs/rnote/network/graph/ICMP报文格式.jpg)
...
- UPD 头结构
    ![UPD 头](/docs/rnote/network/graph/udp协议格式.jpg)
...
- 重点: 
	...
	+ 流量控制: 
        - 滑动窗口(rwnd)大小
        - 怕发送方把接收方缓存塞满
        ![发送端](/docs/rnote/network/graph/TCP-流量控制(发送端).jpg "发送端")
        ![接收端](/docs/rnote/network/graph/TCP-流量控制(接收端).jpg "接收端")
	+ 拥塞控制: 
        - 拥塞窗口(cwnd)大小
        - 怕把网络塞满, 主要为了避免包丢失和超时重传
	        ...
	        - BBR 拥塞算法
                ...
                + [拥塞控制原理图](/docs/rnote/network/graph/拥塞控制bbr.png)
    ```
...
- 握/挥手: 
	+ 三次握手
        ![三次握手](/docs/rnote/network/graph/TCP-三次握手.jpg "三次握手")
	+ 四次挥手
        ![四次挥手](/docs/rnote/network/graph/TCP-四次挥手.jpg "四次挥手")
- 包的序号: 
...
- 概念
	...
	+ root CA
- https协议:([建立连接完整流程](/docs/rnote/network/graph/https建立连接.jpg))
    ![协议](/docs/rnote/network/graph/https协议.jpg)
...
- 负载均衡
	...
	+ DNS也可做内部负载均衡(SLB, Server Load Balancer)
		...
		```
- DNS 解析流程
    ![解析流程](/docs/rnote/network/graph/DNS解析流程.jpg)
...
- CDN分发网络
    ...
    ![网络架构](/docs/rnote/network/graph/CDN 分发系统架构.jpg)
- 有cdn网络的域名解析
...
- 数据中心 : [三层网络结构图](/docs/rnote/network/graph/数据中心架构-三层网络结构.jpg)
    ![基础架构图](/docs/rnote/network/graph/数据中心架构-基础.jpg)
    + 机架(Rack)
    + TOR(Top Of Rack)交换机
    ...
    + 高可用
        ...
        - TOR\汇聚层 交换机高可用
            + 利用交换机的堆叠技术, 将多个交换机形成一个逻辑的交换机。
                ...
                ```
    + 大二层 : [大二层架构图](/docs/rnote/network/graph/数据中心架构-大二层.jpg)
...
- 数据中心流量分类
    ...
    + 东西流量
        ...
        - 方案
            ...
            ![叶脊网络架构图](/docs/rnote/network/graph/数据中心架构-叶脊网络.jpg)
    ...
    + 协议簇
        ...
        - 组件
            + IKE组件 : Internet Key Exchange, 对称密钥交换
                - [Diffie-Hellman算法](/docs/rnote/network/graph/VPN-IKE-Diffie-Hellman对称密钥算法.jpg)
...
- MPLS VPN
    ...
    + VPN 报文转发采用两层标签方式: 
        ...
        ![两层标签转发](/docs/rnote/network/graph/MPLS VPN报文转发(两层标签).jpg)
    + MPLS VPN 的包发送过程: 
...
- [网络结构图](/docs/rnote/network/graph/手机上网/4G网络.jpg)
- 网络演化
    + 2G
...
- 虚拟网卡
    ...
    ![虚拟网卡原理图](/docs/rnote/network/graph/虚拟网卡原理.jpg)
- 虚拟网卡接入云(数据中心)
    + 问题
        - 共享
            + 多个虚拟网卡如何共享同一个物理网卡
    ...
    + 虚拟网卡访问外部
        - 桥接
            ...
            + 这种方式, 不但解决了同一台机器的互通问题, 也解决了跨物理机的互通问题, 因为都在一个二层网络里面, 彼此用相同的网段访问就可以了。但是当规模很大的时候, 会存在问题。
                ...
                - 所以只在小型网络里使用这种方式
            + [桥接原理图](/docs/rnote/network/graph/虚拟网卡-桥接.jpg)
            + [桥接原理图](/docs/rnote/network/graph/虚拟网卡-桥接.jpg)
        - NAT
            ...
            + [NAT原理图](/docs/rnote/network/graph/虚拟网卡-NAT.jpg)
                - [NAT-NO_DHCP](/docs/rnote/network/graph/虚拟网卡-NAT-NO_DHCP.jpg)
- 隔离方案
    + VLAN
        - 命令: vconfig 可以创建带 VLAN 的虚拟网卡
        - 不足: 不能满足大规模云平台能够满足的, 一个是 VLAN 的隔离, 数目太少(VLAN ID最多4096个), 另外就是配置不够灵活。
        ![隔离原理图](/docs/rnote/network/graph/虚拟网卡-VLAN隔离.jpg)
...
- 主题
    + 云中网络隔离、控制
- 软件定义网络(SDN) : ([架构图](/docs/rnote/network/graph/SDN-架构.jpg))
    + 定义 和 功能
    ...
    + 特点: 
        ...
        - 逻辑上的集中控制
- 一种开源实现: OpenFlow + OpenvSwitch : ([框架图](/docs/rnote/network/graph/SDN-开源实现方案.jpg))
...
- SDN控制器如何通过OpenFlow协议控制网络
    + 在 OpenvSwitch 里面, 有一个流表规则, 任何通过这个交换机的包, 都会经过这些规则进行处理, 从而接收、转发、放弃。
        - ![ovs功能](/docs/rnote/network/graph/OpenFlow流表规则.jpg)
    + 实验
        + 常用命令
...
- 如何在云计算中使用 OpenvSwitch
    ...
    ![云计算有无ovs对比](/docs/rnote/network/graph/SDN-云计算中有无OpenvSwitch对比.jpg)
- OpenvSwithch
    + ![架构](/docs/rnote/network/graph/OpenvSwitch-架构图.jpg)
    + ![表结构](/docs/rnote/network/graph/OpenvSwitch-配置表结构.jpg)
    + [表结构json格式](/docs/rnote/network/graph/OpenvSwitch-配置表结构(json).jpg)
...
- 控制网络QoS的方式
    ...
    + 基于类别的排队规则(Classful Queuing Disciplines)
        - 分层令牌桶规则(HTB, Hierarchical Token Bucket)
            ...
            + HTP结构示意图
                ![HTP结构示意](/docs/rnote/network/graph/QoS-流量控制-HTB tree.jpg)
- 如何控制QoS
    + 以OpenvSwitch为例 : [结构示意图](/docs/rnote/network/graph/QoS-流量控制-HTB in OpenvSwitch.jpg)
        - 设置Ingress policy
...
- Overlay网络技术
    ...
    + VXLAN
        ...
        - VXLAN 不是点对点的, 而是支持通过组播(ARP)的来定位目标机器的, 而非一定是这一端发出, 另一端接收。
            ...
            + [VTEP加入组播框架图](/docs/rnote/network/graph/VXLAN-VTEP加入组播.jpg)
- 概述
        ...
        - 物理机 A 上的容器如何访问到物理机 B 上的容器呢
            + Flannel 的方案: 
                - UDP : 使用 UDP 实现 Overlay 网络 
                    ...
                    + ![](/docs/rnote/network/graph/flannel-overlay-udp.jpg)
                - VXLAN 
                    + 在内核态封装, 性能更好一些
                    ...
                    + ![](/docs/rnote/network/graph/flannel-overlay-vxlan.jpg)
            + Flannel 还有: 
...
- Calico 思路
    ...
    ![原理图](/docs/rnote/network/graph/calico-原理图.jpg)
- Calico 网络的转发细节
    ![转发细节](/docs/rnote/network/graph/calico-框架及原理.jpg)
...
- Calico 架构
    ...
    + 安全策略组件 Network Policy
        ...
        - ![Network Policy](/docs/rnote/network/graph/calico-network policy.jpg)
- Calico 存在的问题
    + 全连接复杂性与规模问题
        - BGP 全连接的复杂性问题
...
- rpc框架
    ![rpc框架](/docs/rnote/network/graph/rpc框架.jpg)
    ![rpc框架详述](/docs/rnote/network/graph/rpc框架(详情).jpg)
...
- 8. 从数据中心进网关, 公网 NAT 成私网
    ...
    ![公网NAT成私网](/docs/rnote/network/graph/数据包从公网 NAT 成私网.jpg)
...
- 环境
    ...
    + Open vSwitch 的实验
        ...
        - [实验详情文档](https://github.com/popsuper1982/tcpipillustrated/blob/master/Openvswitch%E5%AE%9E%E9%AA%8C%E6%95%99%E7%A8%8B.pptx)
            + 重点: 一、五、八、十、十一、十五
    + 网络结构图
    ![网络结构图](/docs/rnote/network/graph/试验环境框架.jpg)
...
- 18、TCP 的 BBR 听起来很牛, 你知道它是如何达到这个最优点的嘛？
    ...
    + [BBR拥塞控制](/docs/rnote/network/graph/拥塞控制bbr.png)
...
- 常用工具
    ...
    + ncat(net cat)
- network 知识图谱
    + [知识图谱](/docs/rnote/network/rsc/网络协议知识图谱.jpg)
