---
title: PE 文件签名与数字证书原理详解
description: 深入解析 Windows PE 文件的数字签名机制、证书链验证过程及其在软件安全中的作用。
keywords: PE 签名, 数字证书, 证书链, 代码签名, Windows 安全
---
# PE 签名
...
*注：参考 [调试服务时的自签名证书生成 (Win10)](#_11-2-调试服务时的自签名证书生成-win10)*
...
### 11.2 调试服务时的自签名证书生成 (Win10)
在 Windows 10 环境下调试服务或驱动时，如果遇到证书无效的问题，可以按照以下步骤生成并配置测试证书：

1. **制作证书**：使用 `MakeCert` 命令创建一个测试证书（例如 `Contoso.com(Test)`）。
   ```powershell
   makecert -r -pe -ss PrivateCertStore -n CN=Contoso.com(Test) ContosoTest.cer
   ```

2. **开启测试模式**：以管理员权限运行以下命令，并重启系统。
   ```powershell
   bcdedit /set testsigning on
   ```

3. **导入信任列表**：将生成的证书导入受信任的根证书颁发机构。
   ```powershell
   CertMgr.exe /add ContosoTest.cer /s /r localMachine root
   ```

4. **对文件签名**：使用 `SignTool` 对目标文件进行签名，并建议添加时间戳。
   ```powershell
...
5. **验证签名**：
   ```powershell
   signtool verify /v /kp [service-new.exe]
   ```

> **提示**：以上工具（MakeCert, CertMgr, SignTool）通常包含在 Windows SDK 或 Visual Studio 的命令提示符（Command Prompt）中。
...
+ **Counter Signature** 是对哪块内容计算签名内容呢？
    - 这个 signature 是父签名的一个属性，表达这个属性的结构体可以参考以下定义：
        ```cpp
...
- [Microsoft Docs: Test-Signing](https://docs.microsoft.com/zh-cn/windows-hardware/drivers/install/test-signing)